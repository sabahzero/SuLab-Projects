...
b_GeneCount <- subset(GeneCount, n > 1) # 100 show up more than once
b_GenePlot <- ggplot(b_GeneCount, aes(x = GENE_SYMBOL, y = n)) + 
  geom_bar(stat="identity", fill = "#0073C2FF") +
  theme_pubclean() 
ggsave("b_GenePlot.png", dpi = 500, width = 4, height = 4)
c_GeneCount <- subset(GeneCount, n > 2) # 35 of those show up more than twice
c_GenePlot <- ggplot(c_GeneCount, aes(x = GENE_SYMBOL, y = n)) + 
  geom_bar(stat="identity", fill = "#0073C2FF") +
  theme_pubclean() 
ggsave("c_GenePlot.png", dpi = 500, width = 4, height = 4)

DiseaseCount <- count(ClinGen, DISEASEID_MONDO) # Frequency by Disease
b_DiseaseCount <- subset(DiseaseCount, n > 1) # 55 show up more than once
b_DiseasePlot <- ggplot(b_DiseaseCount, aes(x = DISEASEID_MONDO, y = n)) + 
  geom_bar(stat="identity", fill = "#FFA500") +
  theme_pubclean() 
ggsave("b_DiseasePlot.png", dpi = 500, width = 4, height = 4)
c_DiseaseCount <- subset(DiseaseCount, n > 20) # 7 of those more than 20x 
c_DiseasePlot <- ggplot(c_DiseaseCount, aes(x = DISEASEID_MONDO, y = n)) + 
  geom_bar(stat="identity", fill = "#FFA500") +
  theme_pubclean() 
